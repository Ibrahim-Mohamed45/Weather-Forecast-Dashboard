...
// Creates an empty array to store the data.
let weatherArr = []; let weatherList = [];
let cityName;
...
$("#search-button").on("click", function (e) {
  ...
  // Formats user's input so only first letter is capitalized.
...
function generateWeatherData(cityName) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (overwrite === false) {
        var histoyBtn = $(
          "<button type='submit' class='btn mx-1 mt-3 btn-info'>"
        ).text(cityName);
...
// Function to create today card getting data from the object.
  function currentWeather() {
    // Empties the section before adding new elements to avoid adding more then 1 card.
    $("#today").empty();
    ...
    windText.text(`Wind: ${todayForecast.wind.speed.toFixed(2)} m/s`);
  ...
  // Function that is used to get the date of the objects in the weatherlist array. 
  function getDate(str) {
    var dateStr = str.substring(0, str.indexOf(" "));
    return dateStr.split("-").reverse().join("/");
  }

  // Creates empty arrays for each of days of the forecast.
  var day1 = [];
  var day2 = [];
  var day3 = [];
  var day4 = [];
  ...
  // Function to split the weather data into the 5 days using the get date function.
  function splitIntoFiveDays() {
    // Empties the section before adding new elements to avoid adding more then 5 cards.
    $("#forecast").empty();
    ...
    // 5 for loops that loop through the respective days to get min & max temp, avg wind, avg humidity and the icon at 12pm of each day.
    ...
    // For loop to loop through the above arrays to create 5 cards.
    for (var i = 0; i < 5; i++) {
      ...
      let forecastCard = $(
        `
                  ...
                  <p class="card-text">Wind: ${avgWind[i]} m/s</p>
...
// Function to check local storage for saved search history.
...
// Checks if any previous search data exists in local storage
if (searchHistory !== undefined || searchHistory.length !== 0) {
  displayPreviousSearches();
}

// When a user clicks on a button in the search history it redisplays the information for that city.
